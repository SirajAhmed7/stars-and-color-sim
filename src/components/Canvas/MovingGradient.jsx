...
async function runMovingGradient(canvas) {
  const ga = (a) => (ga.q = []).push(a);
  ga("create", "UA-105392568-1", "auto");
  ga("send", "pageview");

  // Dynamically import GUI
  ...
  function startGUI() {
    gui
      .add(config, "COLOR_UPDATE_SPEED", 0, 20)
      .name("color update speed")
    ...
    bloomFolder.add(config, "BLOOM_INTENSITY", 0.0, 2.0).name("intensity");
...
`
  ...
  let ditheringTexture = createTextureAsync("/images/webgl/LDR_LLL1_0.png");
  ...
  function updateShaderMouseMove() {
    let lastMouseMove = { x: 0, y: 0 };
    let smoothMouse = { x: 0, y: 0 };
    let randomCoordinates = { x: 0, y: 0 };
    let mouseMoving = false;
    let stopTimeout;
    let lerpFactor = 1;
    const minLerpFactor = 0.07;
    const maxLerpFactor = 1;
    const incrementLerpFactor = 0.01;
    const randomInterval = 500;
    ...
    function generateRandomCoordinates() {
      randomCoordinates.x = Math.random() * window.innerWidth;
      randomCoordinates.y = Math.random() * window.innerHeight;
    }
    ...
    function updateCoordinates() {
      ...
      if (!mouseMoving) {
        lerpFactor = minLerpFactor;
      } else {
        lerpFactor = Math.min(maxLerpFactor, lerpFactor + incrementLerpFactor);
      }

      smoothMouse.x = lerp(
        smoothMouse.x,
        mouseMoving ? lastMouseMove.x : randomCoordinates.x,
        lerpFactor
      );
      smoothMouse.y = lerp(
        smoothMouse.y,
        mouseMoving ? lastMouseMove.y : randomCoordinates.y,
        lerpFactor
      );

      pointer = pointers[0];
      let posX = scaleByPixelRatio(smoothMouse.x);
      let posY = scaleByPixelRatio(smoothMouse.y);
    ...
    touchMoveHandler = (e) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        ...
        lastMouseMove.x = touches[i].pageX;
        lastMouseMove.y = touches[i].pageY;
  ...
  mouseCleanup = updateShaderMouseMove();
...
const MovingGradient = () => {
  ...
  useEffect(() => {
    ...
    // Don't run on mobile unless it's homepage
    ...
    runMovingGradient(canvas).then((cleanup) => {
  ...
  return (
    <canvas
      ref={canvasRef}
      className="moving-gradient absolute -z-20 top-0 left-0 h-full w-full pointer-events-none"
    />
  );
};
